...
const convertHandbook = () => {
    ...
    for(token of tokens) {
        switch(token.type) {
            ...
            case "text":
                const linkRegex = /\[([^\[\]]+)\]\(([^)]+)\)/g;
                let matches = token.text.toString().match(linkRegex);
                if(matches) {
                    for(match of matches) {
                        const linkSegments = match.split('(');
                        const anchor = linkSegments[0].replace('[','').replace(']','');
                        const target = linkSegments[1].replace(')','');
                        token.text = token.text.replace(match, `<a href="${target}">${anchor}</a>`);
                    }
                }
                const boldRegex = /\*\*[a-zA-Z0-9\- ]+\*\*/g;
                matches = token.text.toString().match(boldRegex);
                if(matches) {
                    for(match of matches) {
                        token.text = token.text.replace(match, `<span class="inline-bold">${match}</span>`);
                    }
                    token.text = token.text.replace(/\*\*/g,"");
                }
                const italicRegex = /\*[a-zA-Z0-9\- ]+\*/g;
                matches = token.text.toString().match(italicRegex);
                if(matches) {
                    for(match of matches) {
                        token.text = token.text.replace(match, `<i>${match}</i>`);
                    }
                    token.text = token.text.replace(/\*/g,"");
                }

                const slackRegex = /\`#[a-zA-Z0-9]+\`/g;
                matches = token.text.toString().match(slackRegex);
                if(matches) {
                    for(match of matches) {
    ...
    for (section of handbookSections) {
        ...
        `<div id="${section.text.toLowerCase().replace(/ /g,"-")}" class="hbook-title">${section.text}</div> \n` +
